...
`Cicero` was developed as algorithm to create a link between chromatin accessibility and gene expression. 
...
***
The first step that must be taken is to create a object of the ***cell_data_set (CDS)*** class. This format provides a interface defined from the Bioconductor ***SingleCellExperiment*** class. Three input files are required. 
...
This ***expression_matrix*** contains a binary information: is this site open in this cell. In other words: have we found an open chromatin site or have we found a fragment in the sc-ATAC-Seq analysis at this site in this cell. 
...
# save current status of jupyter notebook 
save.image()
ls()
# restart kernel 
```
...
```R
# one can discard variables cds_object & umap_coords
rm("umap_coords")
...
The gene activity scores are used to get a sense of the accessibility of a promoter and it's associated distal elements. 
...
Extract the information of the annotated sites of the cds_object to the variable ***sites***. After changing the format to a data frame, the unused columns can be discarded. Only the information ***gene*** and ***site_name*** (thus peak) is important for the purposes of this workpackage. 
...
This data frame is then stored as .csv file in the directory `/mnt/workspace_stud/stud10/cds_sites_lung.csv`.
